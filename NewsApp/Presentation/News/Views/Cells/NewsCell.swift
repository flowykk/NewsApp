...
final class NewsCell: UITableViewCell {
    ...
    func set(with article: Article) {
        ...
        guard let urlToImageString = article.urlToImage, URL(string: urlToImageString) != nil else {
        ...
        if let savedImage = FileManagerImageStorage.shared.getImage(
        ...
        SwiftDataArticlesStorage.shared.articleExists(withUrl: article.url ?? " ") { [weak self] exists in
...
extension NewsCell {
    ...
    private func favouriteButtonTapped() {
        ...
        if isFavourite {
            SwiftDataArticlesStorage.shared.removeArticle(article: article)
        } else {
            SwiftDataArticlesStorage.shared.saveArticle(article: article)
        }
